...
| S7 | 构建唯一 Linux 产物并从产物完成真实核心链路，生成发布证据 | `In Progress` | — | 已选择 x86_64 AppImage；`electron-builder@26.15.3`、`package:linux`、`verify:linux` 与脱敏报告/截图验证器已落地；候选 `64b4534` 已通过 launch、版本、project/cwd、probe、高思考设置与真实 tool，abort 后 runtime 回到 ready，但工具在 abort RPC 生效前已完成，旧断言因没有 error tool 报 `E_ABORT_FAILED_TOOL`；当前验证器要求无 pending/running tool，并接受新增 failed tool 或 assistant `stopReason=aborted`，需在新的干净 commit 上重跑完整链路 |
