...
| S7 | 构建唯一 Linux 产物并从产物完成真实核心链路，生成发布证据 | `In Progress` | — | 已选择 x86_64 AppImage；`electron-builder@26.15.3`、`package:linux`、`verify:linux` 与脱敏报告/截图验证器已落地；候选 `eecc22d` 已通过 launch、版本、project/cwd、probe、高思考设置与真实 tool，并取得 Pi abort 成功响应、runtime 回到 ready，但当前 run 的 tool card 因 Pi 未发 `tool_execution_end` 而残留为 running，报 `E_ABORT_SETTLED`；当前 kernel 在统一 settle 边界将本轮遗留的 pending/running tool 归一化为 error，需在新的干净 commit 上重跑完整链路 |
