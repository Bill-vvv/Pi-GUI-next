...
| S7 | 构建唯一 Linux 产物并从产物完成真实核心链路，生成发布证据 | `In Progress` | — | 已选择 x86_64 AppImage；`electron-builder@26.15.3`、`package:linux`、`verify:linux` 与脱敏报告/截图验证器已落地；候选 `91ba7a9` 已进一步通过真实 tool 与 abort，在 crash 前因 AppImage 启动器 PID 不是稳定祖先而报 `E_PI_PROCESS_MISSING`；当前验证器改用验证专属 project cwd 加精确 `--mode rpc --offline` 参数唯一识别 Pi，需在新的干净 commit 上重跑 crash/resume/reopen 链路 |
