...
| S7 | 构建唯一 Linux 产物并从产物完成真实核心链路，生成发布证据 | `In Progress` | — | 已选择 x86_64 AppImage；`electron-builder@26.15.3`、`package:linux`、`verify:linux` 与脱敏报告/截图验证器已落地；候选 `e278359` 已通过 launch、版本、project/cwd、probe、高思考设置与真实 tool，中止请求后 runtime 回到 ready 且无运行中 tool，但当前投影没有独立 abort marker，因而旧验证器报 `E_ABORT_SETTLED`；当前验证器在确认 UI 中止入口可用且 tool 正在运行后，直接等待 preload `abort` 的成功 RPC 响应，再验证 runtime/UI 终态一致，需在新的干净 commit 上重跑完整链路 |
