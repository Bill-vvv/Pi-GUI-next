...
- `LOCALAPPDATA` must be present as an exact absolute Windows drive-letter path; missing, relative, direct UNC, device-namespace or malformed values fail startup clearly. Before ProjectStore file I/O, Windows resolves each existing config/state home, requires the canonical path to equal the configured path, and requires every drive root to report `System.IO.DriveInfo.DriveType.Fixed` through fixed system PowerShell. It then creates the `pi-gui-next` directories and repeats the canonical fixed-drive check on those exact directories. Missing PowerShell, probe failure, aliases/reparse redirects, mapped/network, removable and other non-fixed types fail without fallback. This source path is not yet verified on Windows, and it does not eliminate a post-start directory-swap race.
...
### Windows Project path safety

The Windows bootstrap applies this Project/Task Runtime workspace contract:

1. Accept only exact absolute Windows drive-letter paths. Relative paths, drive-relative paths, direct UNC paths, Win32 device namespace paths and control characters fail before filesystem access.
2. Normalize separators and drive-letter casing, resolve the path with `realpath`, then apply the same drive-letter policy to the resolved path. A junction that resolves to UNC is therefore rejected; an accepted local alias is stored under its final canonical drive-letter identity.
3. Require the selected and resolved drive roots to report `System.IO.DriveType.Fixed` through the exact system-PowerShell path under `%SystemRoot%`; probe failure and every non-fixed result fail without fallback. If `realpath` changes the drive root, probe the resolved root separately.
4. Validate the exact canonical workspace before the Pi version subprocess and validate it again immediately before the Pi RPC spawn. This covers ordinary start/activation as well as reload and automatic restart-continuation paths that do not all pass through the same command dispatcher. The second check prevents a junction drift performed during the version probe from reaching RPC spawn.
5. The two pathname checks do not claim a handle-bound `cwd`; a swap between the final check and child creation, or a same-path directory replacement that preserves canonical text, remains part of Windows-native adversarial testing.
6. Keep the existing Linux ProjectStore and Runtime behavior, adding the same two canonical checks around version/RPC child creation.
7. Keep Project path search Linux-only for now. Main rejects Windows and macOS search with `E_PROJECT_PATH_SEARCH_UNAVAILABLE` before Project validation or traversal because the current search depends on Linux `O_DIRECTORY`, `O_NOFOLLOW` and descriptor-pinned `/proc/self/fd` paths.
8. Do not replace descriptor-pinned traversal with `lstat` followed by ordinary pathname recursion. Windows path-search parity requires a native handle implementation that opens reparse points without following them and traverses relative to owned directory handles.
9. Treat Git worktree reads and mutations as a separate capability. This Project policy does not make the current Linux `O_NOFOLLOW` Git implementation Windows-safe.
10. Windows startup rejects noncanonical ProjectStore home/app-directory aliases before Task creation. Linux custom XDG state homes can still contain a symlinked ancestor and produce a Task identity that later canonical checks reject; P4-W2 must resolve that existing behavior without silently rewriting persisted Task identities. A ProjectStore directory swapped after the Windows startup check remains a native handle-boundary risk.

P4-W1 may open and run an explicitly selected Project on a Windows-native verified fixed drive without Project path search. P4-W2 file-search and Task-path parity remain blocked on the native reparse-point-safe handle boundary, Task identity work and Windows-native adversarial validation.
...
| Project path safety | Main Project services + Runtime child-spawn gates | `verified_current`: existing descriptor-pinned `/proc/self/fd` search and canonical Project behavior; `local_source_snapshot`: Runtime checks the canonical workspace before both version and RPC child creation | `local_source_snapshot`: Project/Task workspaces require canonical drive-letter paths before and after `realpath`; direct UNC, device namespace, drive-relative and control-character paths fail; every accepted drive root must report `DriveType.Fixed`; drift during the version probe blocks RPC; Project path search returns `E_PROJECT_PATH_SEARCH_UNAVAILABLE`; final check-to-spawn race and noncanonical Task state-home identity remain Gate items | `identified`: local canonical path policy plus native symlink-safe search boundary | Project selection, Runtime launch/recovery, search, trust or filesystem boundary changes | Reject direct UNC/device/junction-to-UNC/canonical drift and non-fixed drives; prove version-probe drift cannot reach RPC; characterize the residual reparse race and Task home aliases; later native search must survive reparse swaps while remaining inside the owned root |
