...
| Pi package-root discovery | Main Runtime and Package consumers | `verified_current`: canonical executable resolves to the Pi package `dist/cli.js` path | `identified`: an npm `.cmd` shim is not itself the package entry and cannot be used with the current two-parent-directory derivation | `planned` | Pi installation method, package layout or SDK import changes | Resolve and validate the exact package manifest, version and root export without trusting an unrelated shim target |
